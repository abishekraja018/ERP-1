    ...
    if request.method == 'POST':
        if form.is_valid():
            try:
                ...
                messages.error(request, "Could not add event: " + str(e))
        else:
...
def edit_event(request, event_id):
    ...
    context = {'form': form, 'page_title': 'Edit Event'}
    ...
    if request.method == 'POST':
        if form.is_valid():
            try:
                ...
                messages.error(request, "Could not update event: " + str(e))
        else:
            messages.error(request, "Please fill all required fields correctly")
...
def delete_event(request, event_id):
    ...
    try:
        ...
        messages.error(request, "Could not delete event: " + str(e))
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            try:
                ...
                return redirect(reverse('manage_course_allocation'))
            except Exception as e:
                messages.error(request, "Could not create allocation: " + str(e))
        else:
            messages.error(request, "Please fill all required fields correctly")
    else:
...
def edit_course_allocation(request, allocation_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            try:
                ...
                messages.error(request, "Could not update allocation: " + str(e))
        else:
            messages.error(request, "Please fill all required fields correctly")
    else:
    ...
    context = {
        ...
        'page_title': 'Edit Course Allocation'
    }
    return render(request, 'hod_template/add_course_allocation_template.html', context)


def delete_course_allocation(request, allocation_id):
    allocation = get_object_or_404(CourseAllocation, id=allocation_id)
    try:
        allocation.delete()
        messages.success(request, "Allocation deleted successfully!")
    except Exception as e:
        messages.error(request, "Could not delete allocation: " + str(e))
    
    return redirect(reverse('manage_course_allocation'))


def assign_students_to_allocation(request, allocation_id):
    allocation = get_object_or_404(CourseAllocation, id=allocation_id)
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            CourseAllocationStudent.objects.filter(allocation=allocation).delete()
            
            # Add selected students
            for student_id in student_ids:
                student = get_object_or_404(Student, id=student_id)
                CourseAllocationStudent.objects.create(allocation=allocation, student=student)
            
            messages.success(request, "Students assigned successfully!")
            return redirect(reverse('manage_course_allocation'))
        except Exception as e:
            messages.error(request, "Could not assign students: " + str(e))
    
    # Get all students from the same course
    students_in_course = Student.objects.filter(course=allocation.course)
    allocated_students = CourseAllocationStudent.objects.filter(allocation=allocation).values_list('student_id', flat=True)
    
    context = {
        'allocation': allocation,
        'students': students_in_course,
        'allocated_student_ids': list(allocated_students),
        'page_title': 'Assign Students to Allocation'
    }
    return render(request, 'hod_template/assign_students_allocation.html', context)